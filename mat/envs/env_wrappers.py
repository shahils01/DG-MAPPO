...
class ShareVecEnv(ABC):
    ...
    def get_edge_index_matrix(self, faulty_node=None):
        """
        Get the Edge_index_matrix for all the agents (SMAC2 Env)
...
class ShareDummyVecEnv(ShareVecEnv):
    ...
    def get_edge_index_matrix(self, faulty_node=None):
        ...
        return edge_index_matrix.transpose(1,0,2)
    ...
    def get_visibility_matrix(self):
        ...
        return visibility_matrix.transpose(1,0,2)
    ...
    def render(self, mode="human"):
        if mode == "rgb_array":
            ...
            for env in self.envs:
                ...
                try:
                    frames.append(target.render(mode=mode))
                except TypeError:
                    frames.append(target.render())
            return np.array(frames)
        elif mode == "human":
            for env in self.envs:
                target = getattr(env, "unwrapped", env)
                try:
                    target.render(mode=mode)
                except TypeError:
                    target.render()
        else:
            raise NotImplementedError
