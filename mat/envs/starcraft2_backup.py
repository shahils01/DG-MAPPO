...
class StarCraft2Env_(MultiAgentEnv):
    ...
    def __init__(
        ...
        print('self.capability_config = ', self.capability_config)

        # self.stochastic_attack = "attack" in self.capability_config
        self.stochastic_attack = False
    ...
    def get_agent_action(self, a_id, action):
        ...
        ), "Agent {} cannot perform action {} because available actions are {}".format(a_id, action, avail_actions)
    ...
    def get_edge_index_matrix(self):
        ...
        for agent_id in range(n_agents):
            ...
            # Source nodes are the agent_id, target nodes are visible units
            '''num_edges = len(visible_units)
            edge_index = np.zeros((2, num_edges+1), dtype=np.int64)
            edge_index[0, :] = agent_id  # Source node (current agent)
            edge_index[1, 1:] = visible_units  # Target nodes (visible units)
            edge_index[1, 0] = agent_id  # Self Loop'''

            '''if agent_id == 0:
                edge_indices.append(edge_index)
                edge_indices = np.array(edge_indices).squeeze(0)
            else:
                edge_indices = np.concatenate((edge_indices, edge_index), axis=-1)'''
