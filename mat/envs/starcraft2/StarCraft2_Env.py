...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        replay_dir="/home/shahils/Desktop/marl_ws/Multi-Agent-Transformer/mat/scripts",
        replay_prefix="",
        window_size_x=1920,
        window_size_y=1200,
        heuristic_ai=False,
        ...
        self.strict_local_obs = args.strict_local_obs
    ...
    def step(self, actions):
        ...
        dones = np.zeros((self.n_agents), dtype=np.bool_)
    ...
    def get_obs_agent(self, agent_id):
        ...
        '''print('ally_feats_dim = ', ally_feats_dim)
        print('enemy_feats_dim = ', enemy_feats_dim)
        print('move_feats_dim = ', move_feats_dim)
        print('own_feats_dim = ', own_feats_dim)'''
        ...
        ally_id_feats = np.zeros(self.n_agents, dtype=np.float32)
        ...
        if self.obs_agent_id:
            ...
            if self.strict_local_obs:
                agent_obs = np.concatenate((enemy_feats.flatten(),
                                            move_feats.flatten(),
                                            own_feats.flatten(),
                                            agent_id_feats.flatten(),
                                            ally_id_feats.flatten()))
            else:
                agent_obs = np.concatenate((ally_feats.flatten(),
                                          enemy_feats.flatten(),
                                          move_feats.flatten(),
    ...
    def get_obs_size(self):
        ...
        if self.strict_local_obs:
            all_feats = move_feats + enemy_feats + own_feats
        else:
        ...
        if self.strict_local_obs:
            ally_id_feats = self.n_agents
            all_feats += ally_id_feats
    ...
    def get_edge_index_matrix(self, faulty_node=None):
        ...
        visibility_matrix = visibility_matrix[:,:self.n_agents]
    ...
    def render(self, mode="human"):
        if self.renderer is None:
            from mat.envs.starcraft2.render import StarCraft2Renderer
