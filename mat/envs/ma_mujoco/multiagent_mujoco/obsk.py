    ...
    for c in global_categories:
        ...
        for j in global_dict.get("joints", []):
            if c in j.extra_obs:
                ...
                items = getattr(env.unwrapped.data, c)[getattr(j, "{}_ids".format("qvel"))]
...
def build_visibility_matrix(parts, edges, include_self=False):
    """
    Build an agent-level visibility matrix from:
      - parts: list of tuples of Node objects, defining agents
      - edges: list of HyperEdge objects (with .edges as a set of Nodes)

    Visibility rule:
      Two agents i, j are visible to each other iff they have at least one
      Node that co-appears in some HyperEdge.
    ...
    edges : list[HyperEdge]
        Each HyperEdge has attribute `.edges` which is a set of Node objects.
    include_self : bool
        If True, vis[i, i] = 1 for any agent that appears in some edge.
        If False, diagonal remains 0.

    Returns
    -------
    vis : np.ndarray of shape (num_agents, num_agents), dtype=int
    """
    # Map each Node -> agent index
    node2agent = {}
    for agent_idx, group in enumerate(parts):
        for node in group:
            node2agent[node] = agent_idx

    n_agents = len(parts)
    # vis = np.zeros((n_agents, n_agents), dtype=int)
    vis = np.eye(n_agents, dtype=int)

    for he in edges:
        # he.edges is a set of Node objects
        agents_in_edge = sorted(
            {node2agent[n] for n in he.edges if n in node2agent}
        )

        # Fully connect all agents that share this hyperedge
        for i in agents_in_edge:
            for j in agents_in_edge:
...
def get_parts_and_edges(label, partitioning):
    if label in ["half_cheetah", "HalfCheetah-v5"]:
        ...
        return parts, edges, globals, build_visibility_matrix(parts, edges)
        ...
        hip1 = Node("hip1", -8, -8, 2) #
        ankle1 = Node("ankle1", -7, -7, 3)#,
        hip2 = Node("hip2", -6, -6, 4)#,
        ankle2 = Node("ankle2", -5, -5, 5)#,
        hip3 = Node("hip3", -4, -4, 6)#,
        ankle3 = Node("ankle3", -3, -3, 7)#,
        hip4 = Node("hip4", -2, -2, 0)#,
        ankle4 = Node("ankle4", -1, -1, 1)#,
        ...
        root = Node("root", 0, 0, -1, extra_obs={"qpos": lambda env: env.unwrapped.data.qpos[:7],
                                                 "qvel": lambda env: env.unwrapped.data.qvel[:6],
                                                 "cfrc_ext": lambda env: env.unwrapped.data.cfrc_ext[0:1]})
        globals = {"joints":[root]}
        ...
        root = Node("root", 0, 0, -1)
        globals = {"joints":[free_joint, root]}
